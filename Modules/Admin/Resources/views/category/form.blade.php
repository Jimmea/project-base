        ...
        <form class="form-horizontal" method="POST" action="{{ $action }}">
            ...
            <section id="general">
                ...
                <div class="form-group">
                    <label class="font-weight-bold">Loại danh mục <span class="color_red">(*)</span></label>
                    <div class="input-group mb-2 mr-sm-2 ">
                        <select name="cat_type" class="form-control">
                            <option value="">Chọn một</option>
                            @foreach($settingGlobal['category_type'] as $key => $name)
                                @php $selected = get_value_field('cat_type', $category) == $key ? 'selected=selected' : '' @endphp
                                <option {{ $selected }} value="{{ $key }}">{{ $name }}</option>
                            @endforeach
                        </select>
                    </div>
                    {!! get_error($errors, 'cat_type') !!}
                </div>

                <div class="form-group">
                    <label class="font-weight-bold">Tên danh mục <span class="color_red">(*)</span></label>
                ...
                <div class="form-group">
                    ...
                    <div class="form-check-group">
                        ...
                        <div>
                            <label class="radio-inline">
                                @php $selected = get_value_field('cat_hot', $category) ? 'checked=checked' : '' @endphp
                                <input type="checkbox" name="cat_seo" {{ $selected }} value="1"> SEO
                            </label>
                        </div>
                    </div>
                </div>
